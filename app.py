...
O provider LLM é substituído pelo backend configurável de llm_provider.py.
"""
...
import NEXUS_COMPLETE
from llm_provider import ConfigurableLLMEngine

# Faz o NexusAgent usar o provider configurável sem alterar a lógica ReAct.
NEXUS_COMPLETE.LLMEngine = ConfigurableLLMEngine
NexusAgent = NEXUS_COMPLETE.NexusAgent

# Setup
app = FastAPI(title="NEXUS Web", description="NEXUS com LLM local/open-weight configurável")
...
async def startup():
    ...
    logger.info(
        ...
        os.getenv("NEXUS_PROVIDER", "ollama"),
        os.getenv("NEXUS_MODEL", os.getenv("OLLAMA_MODEL", "qwen3:8b")),
        os.getenv("NEXUS_BASE_URL", "http://127.0.0.1:11434"),
    )
    agent = NexusAgent()
...
async def llm_config():
    """Mostra a configuração ativa do LLM sem expor a chave da API."""
    return {
        "provider": os.getenv("NEXUS_PROVIDER", "ollama"),
        "model": os.getenv("NEXUS_MODEL", os.getenv("OLLAMA_MODEL", "qwen3:8b")),
        "base_url": os.getenv("NEXUS_BASE_URL", "http://127.0.0.1:11434"),
        "api_key_configured": bool(os.getenv("NEXUS_API_KEY", "")),
        "supported": ["ollama", "vllm", "llama.cpp", "lmstudio", "openai_compatible"],
    }
...
async def get_kb():
    ...
    return {"total": agent.kb.count(), "recent": entries}
...
async def execute_plugin(data: dict):
    ...
    try:
        return await agent.plugins.execute(plugin_name)
    except Exception as e:
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        while True:
            ...
            await websocket.send_json({"type": "status", "message": "🧠 Processando..."})
